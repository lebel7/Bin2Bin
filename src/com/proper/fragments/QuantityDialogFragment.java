...
import com.android.barcode.ActSingleMove;
import com.chainway.ht.ui.ActBinProductMain;
import com.proper.bin2bin.ActBinItemSelection;
import com.proper.bin2bin.R;
import com.proper.data.ProductBinSelection;
import com.proper.data.adapters.BinResponseSelectionAdapter;
...
public class QuantityDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (parent.equalsIgnoreCase("ActSingleMove")) {
            moveItem = ((ActSingleMove)getActivity()).getCurrentBinSelection();     //get data from activity
        }
        //moveItem = ((ActBinProductMain)getActivity()).getCurrentBinSelection();     //get data from activity
        View view = inflater.inflate(R.layout.dialog_quantity, null);
        ScrollView lytMain = (ScrollView) view.findViewById(R.id.lyt_dialogQuantity);
...
//        lytMain.setLayoutParams(params);
        ...
        if (moveItem.getQtyInBin() > 0 && moveItem.getQtyToMove() > 0) {
            ...
            moveQty.setTextColor(Color.parseColor("#000000"));
        }
        if (moveItem.getQtyInBin() == 0 && moveItem.getQtyToMove() > 0) {
        ...
        if (moveItem.getQtyInBin() > 0 && moveItem.getQtyToMove() == 0) {
            ...
            binQty.setTextColor(Color.parseColor("#000000"));
        }
    ...
    public void onClick(View v) {
        ...
        if (v == btnFinish) {
            if (moveItem.getQtyToMove() <= 0) {
                ...
                if (parent.equalsIgnoreCase("ActSingleMove")) {
                    ((ActSingleMove) getActivity()).setCurrentBinSelection(moveItem);    // pass data back to the parent activity
                }
                ICommunicator.onDialogMessage(R.integer.MSG_OK);
                dismiss();
            }
        }
    }
